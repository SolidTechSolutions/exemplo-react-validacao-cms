...
// Example front-end for validating signed CMS/CAdES (.p7s) documents.
//
// Two modes:
// - "backend" (default): talks to the local example backend
//   (exemplo-java-integracao-validacao-cms, POST /api/cms/validate/form). The
...
//
// DETACHED CAdES: when the .p7s does not embed the original document, also
// attach the original file(s), in the same order as the .p7s files.
//
// The report below is intentionally plain (a table of key facts), not a
// reproduction of the Portal SolidSign validator's UI.

const DEFAULT_BACKEND_URL = 'http://localhost:8095';
          ...
          <div key={i}>
            ...
            <table className="report-table">
              <tbody>
                ...
                <Row label="Original content attached" value={sig.hasAttachedOriginalFile} />
...
export default function App() {
  ...
  const submit = async (e) => {
    ...
    if (isDetached && originalDocuments.length === 0) { setError('DETACHED signatures need the original document(s).'); return; }
    ...
    try {
      ...
      if (mode === 'backend') {
        ...
        if (isDetached) originalDocuments.forEach((f) => fd.append('originalFile', f));
        url = `${backendUrl.replace(/\/$/, '')}/api/cms/validate/form`;
      } else {
        documents.forEach((f, i) => fd.append(`signedFile[${i}]`, f));
        if (isDetached) originalDocuments.forEach((f, i) => fd.append(`originalFile[${i}]`, f));
        url = `${baseUrl.replace(/\/$/, '')}/solidsign/dsig/validation/verify-cms`;
      }
  ...
  return (
    <div className="page">
      <h1>Validate CMS/CAdES (React example)</h1>
      <p className="subtitle">
        Example front-end for <code>exemplo-java-integracao-validacao-cms</code>. By default it
      ...
      <form onSubmit={submit} className="form">
        ...
        <fieldset>
          ...
          <label className="mode-toggle" style={{ marginTop: 4 }}>
            <input type="checkbox" checked={isDetached} onChange={(e) => setIsDetached(e.target.checked)} />
            DETACHED signature (original not embedded)
          </label>
          {isDetached && (
            <label>Original document(s) — same order as the .p7s files above
              <input type="file" multiple onChange={(e) => setOriginalDocuments(Array.from(e.target.files))} />
            </label>
          )}
        </fieldset>
      ...
      {docs.length > 0 && (
        <>
          ...
          <DocumentReport doc={docs[selectedIdx]} fileName={documents[selectedIdx]?.name || `document_${selectedIdx + 1}.p7s`} />
        </>
      )}
    </div>
  );
